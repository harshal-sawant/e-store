...
export function wishlistReducer(
  ...
  switch (action.type) {
    case WishlistActions.INITIALIZEWISHLIST:
      let initProducts = (action as WishlistActions.initializeWishlistAction)
        .payload;
    ...
    case WishlistActions.ADD_TO_WISHLIST:
      const newProducts = (action as WishlistActions.addToWishlistAction)
        .payload;
      return { ...store, products: [...store.products, newProducts] };
    case WishlistActions.DELETEWISHLISTITEM:
      let removeId = (action as WishlistActions.deleteWishlistItemAction)
        .payload[1];
    ...
    case WishlistActions.WISHLISTSUCCESS:
      return {
        ...store,
        products: (action as WishlistActions.WishlistSuccessAction).payload,
      };
    default:
      return store;
  }
